...
class SecurityFlavor(Test):
    ...
    def test_security_flavor(self):
        ...
        if not os.path.isfile("/proc/powerpc/lparcfg"):
            self.fail("lparcfg file doesn't exist")
        lparcfg = "/proc/powerpc/lparcfg"
        if not os.path.isfile(lparcfg):
            self.fail("lparcfg proc file doesn't exist")
        cmd = "lparstat -x"

        output = genio.read_file(lparcfg)
        lparcfg_value = output.splitlines()[-1].strip().split("=")[-1]
